...
const getCourses = async (page) => {
    ...
    try {
      const result = await client.query(`SELECT id, course_name, course_link FROM courses LIMIT 5 OFFSET ${offset}`);
      return result.rows;
    } finally {
      client.release();
    }
  ...
  const getCourse = async (id) => {
    ...
    try {
      const result = await client.query(`SELECT id, course_name, course_link FROM courses where id = ${id}`);
      return result.rows;
    } finally {
      client.release();
    }
...
export { getCourses, getCoursesCount, getCourse, deleteCourse, insertData, updateData };
